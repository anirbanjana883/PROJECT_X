...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        // --- MOCK DATA --- (Remove this block when you add patients)
        // This is so the UI looks good for the demo
        if (patients.length === 0) {
            setPatients([
                {_id: '1', name: 'Anirban Das', email: 'test.patient1@gmail.com'},
                {_id: '2', name: 'John Doe', email: 'john.doe@gmail.com'},
            ]);
        }
        // --- End Mock Data ---
            ...
            <ul className="divide-y divide-gray-200 dark:divide-gray-700/50">
              {patients.map((patient) => (
                <li key={patient._id} className="group p-4 flex items-center justify-between hover:bg-gray-50/50 dark:hover:bg-gray-900/20 transition-colors duration-200 rounded-lg">
                  ...
                  <button className="text-sm text-blue-600 dark:text-blue-400 font-semibold hover:underline group-hover:translate-x-1 transition-transform flex items-center gap-1.5">
